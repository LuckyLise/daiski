...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_course'])) {
    ...
    $course_id = $db_host->lastInsertId(); // 取得剛剛插入的課程 ID
    ...
    $stmt = $db_host->prepare("INSERT INTO coursevariants (course_id, type, difficulty, duration, price, location_id, coach_id) 
...
if ($page < 1) $page = 1; // 確保頁數不會小於 1

// 計算要從哪一筆開始抓資料
$offset = ($page - 1) * $limit;

// 取得課程總數，計算總頁數
$total_stmt = $db_host->query("SELECT COUNT(*) FROM course");
...
if (isset($_GET['sort'])) {
    if ($_GET['sort'] === "price_asc") {
        $orderBy = "ORDER BY cv.price ASC"; // 價格從低到高
    } elseif ($_GET['sort'] === "price_desc") {
        $orderBy = "ORDER BY cv.price DESC"; // 價格從高到低
    } elseif ($_GET['sort'] === "created_at") {
        $orderBy = "ORDER BY cv.created_at DESC"; // 依照課程創建時間排序
    }
}
// **查詢課程**
$sql = "SELECT c.name, c.description, c.id,
...
<div class="container">
    ...
    <form method="GET" class="row g-3 ">
        <div class="col-md-3">
            ...
            <input type="text" name="search" class="form-control" placeholder="輸入課程名稱或描述">
        </div>
        <div class="col-md-3">
            <label for="">類型</label>
    ...
    <div class="d-flex justify-content-end my-3">
        <a href="?sort=price_asc" class="btn btn-outline-primary me-2">價格：低 → 高</a>
        <a href="?sort=price_desc" class="btn btn-outline-primary me-2">價格：高 → 低</a>
        <a href="?sort=created_at" class="btn btn-outline-primary me-2">最新課程</a>
            ...
            <?php foreach ($courses as $course): ?>
                <tr>
                    ...
                    <td> <?php
                            ...
                            ?>
                            <img src="<?= htmlspecialchars($images[0]) ?>" width="100" height="100" class="img-thumbnail preview-img"
                    ...
                    <td>
                        ...
                        <div class="modal fade" id="detailsModal-<?= htmlspecialchars($course['id']) ?>" tabindex="-1" aria-labelledby="detailsModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">課程詳情 - <?= htmlspecialchars($course['name']) ?></h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
